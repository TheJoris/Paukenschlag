...
We also looked at the dynamics of the 1D system, similar as was done for the 2D system. Because there exists a Green's function for a reversible system in 1D, we can compare the spatial probability densities for a system with a dissociation reaction with theory. Results and system parameters are given in \fref{newBD1D_rdens_compare}. Again the simulations are in good agreement with theory if we choose the timestep small enough. The reversible system shows an even better agreement with theory. The reason is we choose such a high dissociation rate, we are effectively simulating a reflective system.
