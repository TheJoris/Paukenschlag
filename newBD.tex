...
We want to design a simple Brownian Dynamics algorithm designed for simulating second order chemical reactions of the form
\begin{equation}
...
\elabel{move_prop}
\end{multline}
where $\sigma$ is sum of the radii of the particle pair. 

\subsubsection{Forward move}
Two particles can attempt a reaction when they lie in each other's {\it reaction volume}. The reactiom volume, denoted by $V^*$, is a spherical shell around each particle with a thickness $\delta$. The reaction attempt is accepted with a probability $P^{\rm acc}_{{\rm u}^* \to {\rm b}}$. Thus, two particles initially in the unbound state $u$, separated by an inter-particle vector $\vec{r_0}$, move into the bound state with a probability
\begin{equation}
P_{ \rm{u} \to \rm{b} }(\vec{r_0}) = P^{\rm move}_{{\rm u} \to {\rm u}^*}({\rm u}^*,\Delta t|\vec{r_0}) P^{\rm acc}_{{\rm u}^* \to {\rm b}}.
\end{equation}
here ${\rm u}^*$ denotes the intermediate complex our the state where the inter-particle vector lies inside the reaction volume. 

\subsubsection{Backward move}
The backward move starts with the dissociation of the complex with a probability $k_d \Delta t$. After which the two product particles are placed at a distance $r'$, inside the reaction volume, drawn from the flat distribution $(V^*)^{-1} \theta(\sigma + \delta - r')\theta(r' - \sigma)d\vec{r'}$, where $V^*$ normalizes the distribution. Thus the transition probability from being bound to being in the unbound state $u^*$ at position $\vec{r'}$ is
\begin{equation}
 P^{\rm acc}_{b \to u^*}(\vec{r'})d\vec{r'} = k_d \Delta t \, \frac{\theta(\sigma + \delta - r') \, \theta(r' - \sigma) d\vec{r'}}{V^*}.
\end{equation}
...
\begin{equation}
P_{ \rm{b} \to \rm{u}}(\vec{r})d\vec{r} =  \int_{V} d\vec{r'} P^{\rm acc}_{b \to u^*}(\vec{r'}) P^{\rm move}_{u^* \to u}(\vec{r},\Delta t|\vec{r'})d\vec{r},
\end{equation}
where we integrated over all intermediate positions inside the reaction volume.

\subsubsection{Solving the detailed balance equation}
Now we have defined both transition probabilities, we can solve the detailed balance equation (\eref{DBeqn}) for the acceptance probability of a forward move $P^{\rm acc}_{{\rm u}^* \to {\rm b}}$. We first use the important relation found in \cite{Morelli2008a} for a system in equilibrium
\begin{equation}
...
\end{equation}
Secondly, we show in \aref{newBD:fwdbwdeq} that the movement step in both transition probabilities, apart from a normalization factor, equal and thus cancel. We are left with
\begin{equation}
...
A particle can interact with a plane when the projection of it's center of mass lies in the plane and is separated less than a distance $\delta$. Due to the symmetry of the interaction, it is essentially a 1D problem. Since the particle can interact with both sides of the plane, the reaction volume is $V^*=2 \delta$.
