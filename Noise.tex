...
\end{equation}
The rate at which TF's bind to a sink (ak. never occupied) in an equilibrated system is found to be (\aref{noise:backward_eqn})
\begin{equation}
 k_{\rm rad}(t) = k_+ \, S_{\rm rad}(t|\sigma).
 \elabel{krad}
\end{equation}

Now, Laplace transforming \eref{rev_conv} and solving for $\hat{\mathscr{S}}_{{\rm rev}}$, plugging the result into \eref{corr_func}
