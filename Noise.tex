...
First we compare the noise percieved by a promoter and a receptor when the bulk concentrations $\bar{c}_p$ and $\bar{c}_r$ are equal. Now these factors cancel in \eref{NoiseCmpA}.

A plot of \eref{NoiseCmpA} for different values of $k_d$ and $k_a$ is shown in \fref{TableauxNoiseCmpA} (a). For high values of the DNA dissociation rate $k_d$ (right of the optimum), decreasing the rate (or increasing the association rate) increases the concentration $\bar{\xi}$ of TF's on the DNA, and thereby increasing the number of independent measurements a promoter can make during its integration time. However, as the promoter becomes occupied for more than half of the time, \fref{TableauxNoiseCmpA} (b), it doesn't `measure' the greater part of the incoming particles any more, and the noise starts to rise again. The optimum is thus caused by a tradeoff between a higer concentration on the DNA and a higher promoter occupancy.
...
The reason for this monotonic behavior is that the DNA occupancy $\bar{\xi}$ increases linearly with $1/k_d$, in the inifite system we are looking at. A single particle starting on the DNA, however, will need more and more time to 'escape' with decreasing $k_d$, because it has a higher chance of rebinding, as is shown in \fref{TableauxNoiseCmpB}, (a). Here we plotted the diffusion limited rates $k_D^r/k_D^p L^{-2}$ as a function of $k_d$, showing that a longer DNA residence time tanslates to a lower diffusion limited rate per particle, and therefore a lower escape probability \eref{Sinfty}. Because the effect of increasing $\bar{\xi}$ 'winns' from the decreasing $k_D^p$, the noise decreases monotonic with decreasing $1/k_d$.
...
To capture the effect of the finiteness of the number of transcription factors available, we make the bulk concentration $\bar{c}_p$ dependent on $k_d$. Imagine we draw a box with dimensions $L$ centered around the promoter, and fix te average number of TF's, $TF_0$, inside this box by
\begin{equation}
...
Now, the concentration on the DNA does not increase linearly with $1/k_d$, but saturates as all available particles in the box occupy the DNA. See \fref{TableauxNoiseCmpA}, (b). Now, a new optimum emerges as we decrease $k_d$, because the increase in $\bar{\xi}$ is unable to compensate the decrease of $k_D^p$, \fref{TableauxNoiseCmpA}, (c). Again, the intrinsic rates of the promoter are changed in such a way the average occupancy stays the same when changing the DNA rates. Note that, in reality, the DNA is much longer than the dimension $L$ of the box, and the non-linearity between $\bar{\xi}$ and $1/k_d$ will already emerge at much higer values $k_d$. 

\begin{figure}[hb!]
\centering
\includegraphics[scale=.8]{TableauxNoiseCmpB}
\caption{\flabel{TableauxNoiseCmpB} {\bf a}) Comparing the diffusion limited rate to a promoter with a receptor. For longer DNA residence times, the rate to a promoter decreases. {\bf b}) DNA occupancy $\bar{\xi}$ for a system where the number of particles inside a box around the DNA is kept fixed (solid lines), or where the bulk concentration stays constant (dotted lines). {\bf c}) Comparing the noise in the promoter and the receptor state, this time with the particle number inside a box around the objects kept fixed, \eref{NoiseCmpB}. The noise has an optimum, because the increase of $\bar{\xi}$ can not compensate the decrease of the diffusion limited rate $k_D^p$ for all values of $k_d$. 
}
\end{figure}
