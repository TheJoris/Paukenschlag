...
We will first derive the correlation function, from which we can obtain the correlation time. With the correlation time in hand, we obtain general relations for the noise in the promoter occupancy, concentration fluctuations around the promoter and the lower limit in the noise set by the diffusive arrival. We continue with obtaining expressions in different geometries. First for a system where particles can only move on the DNA (1D only), secondly for DNA in a well mixed solution and end with the full system where TF's can perform multiple rebinding with the DNA before binding to the promoter.
...
\caption{\flabel{NoisePlotFull} Low frequency limit of the power spectrum (\eref{PowPro}) for a promoter site on the DNA, compared with simulations. The grey lines shows the result found in \cite{Tkacik2009}, which is symmetric around $n$.}
\end{figure}
...
\caption{\flabel{GaspervsJoris} Comparing the promoter noise level as defined in \eref{PowPro}, for a system with and without a ultraviolet cutoff. The cutoff was at $\pi/\sigma$. The curve with cutoff is equal to Eq. 68 in \cite{Tkacik2009}, but with a factor $(1-\bar{n})$ in the diffusion part.}
\end{figure}
