...
\end{equation}
where $\mathscr{S}_{\rm rev}(\tau|*)$ is the probability of the promoter to be free at $\tau$, given that it started bounded. The promoter can undergo multiple rounds of binding and unbinding during the time $\tau$. We can describe this reversible process in term of an irreversible one via the convolution \cite{Agmon1990}
\begin{equation}
...
\begin{equation}
 \lim_{c \to 0, s \to 0} \hat{C_n}(s) = \frac{\bar{n} \sigma_n^2}{k_+ \bar{c} \, S_{\rm rad}(\infty|z_0)} = \frac{\sigma_n^2 \tau_c^*}{S_{\rm rad}(\infty|z_0)},
 \elabel{CT1}
\end{equation}
where $\tau_c^*$ is the correlation time for the fast process, $\tau_c^* = k_+ \hat{c} + k_-$. To summarize, given the large time limit of $S(t|z_0)$, we have, in the low $c$ limit, the correlation time.
...
To find the large time limit of the survival probability for \emph{a single} TF starting on the promoter site, we start with writing down the differential equations describing the full system
\setlength{\jot}{12pt}
\begin{equation}
\elabel{DE_full}
\begin{split}
...
\pdiff{P_{3}(z,{\bf r},t|z_0,r_0)}{t} & = D_3 \nabla^2 P_{3}(z, {\bf r}, t|z_0,r_0) \\
 & \quad - \Big[ k_a P_{3}(z, {\bf r}, t|z_0,r_0) - k_d P_{1}(z,t|z_0) \Big] \frac{\delta(|\mathbf r \mathrm| - \sigma)}{2 \pi \sigma}.
\end{split}
\end{equation}
Here $P_1(z, t|z_0)$ is the Green's function describing the 1D sliding of the TF along the DNA, starting on the sink positioned at $z_0$. We model the DNA as an infinite long rod along the z-axis with a radius $\sigma$. The probability exchange between the DNA and bulk happens at a distance $\sigma$ from the z-axis, dictated by the delta function. Excursions in the cytoplasm are described by $P_3(z, {\bf r}, t|z_0,r_0)$. First we Laplace transform with respect to time
\begin{eqnarray*}
...
\begin{eqnarray}
 ...
 \elabel{FDE_bulk}
\end{eqnarray}
Here $q$ is the spatial Fourier variable conjugate to $z$, and ${\bf k}$ is conjugate to ${\bf r}$. We assumed the promoter and initial position both to be at the origin: $z_0 = 0$. Observe that the bulk density $\tilde{P}_3$ appears as a function of $q$ only in the equation for $\tilde{P}_1$. In order to solve for $\tilde{P}_1$, we first solve the second equation for $\tilde{P}_3(q,{\bf k})$, 
\begin{equation}
...
\end{eqnarray}
Solving the above for $\tilde{P}_1(q)$, and substituting into \eref{FDE_dna}, we solve it for $\tilde{P}_1(q)$. Again, back-transforming this equation in $q$, we find
\begin{equation}
...
\end{equation}
Finally, we can solve \eref{FDE_dna2} for $\tilde{\xi}(z_0, s|z_0)$ to obtain the probability density at the promoter site in Laplace space. In the limit $s \to 0$, our expression becomes
\begin{equation}
...
\end{gather}
The $s \to 0$ limit in $k_+ \, \tilde{\xi}(z_0,s|z_0)$ gives the total flux which flowed into the sink, and this relates to the long time limit of the survival probability
\begin{equation}
...
\begin{equation}
 \pdiff{\mathscr{S}_{\rm rad}(t|{\rm eq})}{t} = -\bar{\xi} k_+ \, \mathscr{S}_{\rm rad}(t|\sigma),
\end{equation}
...
\begin{equation}
 \lim_{\bar{\xi}\to 0} \hat{\mathscr{S}}_{\rm rad}(s|\sigma) = \frac{1}{\bar{\xi} k_+} \left[ \frac{\bar{\xi}\,\hat{k}_{\rm rad}(s)}{1 + \bar{\xi}\,\hat{k}_{\rm rad}(s)}\right].
\end{equation}
In the limit of low $s$, which we use when calculating the correlation time, the lowest power of $s$ in $\hat{k}_{\rm rad}(s)$ dominates. Since for $t\to\infty$, $k_{\rm rad}(t)$ approaches the steady state limit $k_{\rm ss}$: $\lim_{s\to0} \hat{k}_{\rm rad}(s) \sim k_{\rm ss}/s = k_+ S_{\rm rad}(\infty|z_0)/s$. Using this in the above equation, we can back-transform to the time domain
\begin{multline}
 \hat{\mathscr{S}}_{\rm rad}(s|\sigma) = \frac{k_{\rm ss}}{k_+} \left[ \frac{1}{s + \bar{\xi}\,k_{\rm ss}} \right] \quad \xrightarrow{\mathscr{L}^{-1}} \\ \mathscr{S}_{\rm rad}(t|\sigma) = S_{\rm rad}(\infty|z_0)\,{\rm exp}\left[ -\bar{\xi} k_+ S_{\rm rad}(\infty|z_0)\,t \right].
\end{multline}
Comparing this result with \eref{Def_Ssi}, we see that the rebinding trajectories are coarse-grained out by replacing $S_{\rm rad}(t|z_0)$ with $S_{\rm rad}(\infty|z_0)$. We are left with a Poisson decay of the survival probability, starting at the probability with which the particle starting at contact escapes. The reason this is a good approximation is due to the timescale seperation between particles comming from the bulk, and the rebinding of the particle at contact. As is shown in \aref{noise:Seq_approx}, \fref{SeqApprx}. Coarse graining out the non-Poissonian behaviour is exactly what van Zon does by renormalizing the reaction rates with the escape probability. Thus, in the low concentration limit of non-interacting TF's, the methods of van Zon and de Ronde to obtain the correlation time are equivalent.
