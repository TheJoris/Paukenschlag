...
\begin{equation}
 ...
 \elabel{DE_Seq}
\end{equation}
which states that the rate at which TF's bind to the promoter is equal to the rate $k_{{\rm irr}}(t)$ at with they would bind to the promoter if it is never occupied, times the probability that the promoter is still unoccupied. $\bar{\xi}$ is the average concentration of TF's on the DNA. Solving the equation yields
\begin{equation}
...
\begin{equation}
 k_{\rm irr}(t) = k_+ \, S_{\rm irr}(t|\sigma).
 \elabel{krad}
\end{equation}
...
\begin{eqnarray}
 \pdiff{\mathscr{S}_{\rm irr}(t|{\rm eq})}{t} & = & -\bar{\xi} k_+ \, S_{\rm irr}(t|\sigma) \, \mathscr{S}_{\rm irr}(t|{\rm eq}) \\
...
\begin{eqnarray*}
 s \, \hat{\xi}(z) - \delta(z - z_0) & = & D_1 \frac{\partial^2\hat{\xi}(z)}{\partial^2 z} - k_+ \hat{\xi}(z) \delta(z - z_0) - k_d \hat{\xi}(z) + k_a \hat{c}(z, |{\bf r}| = \sigma) \\
 s \, \hat{c}(z,{\mathbf r}) & = & D_3 \nabla^2 \hat{c}(z, \mathbf r \mathrm) - \left[ k_a \hat{c}(z, \mathbf r \mathrm) - k_d \hat{\xi}(z) \right] \frac{\delta(|\mathbf r \mathrm| - \sigma)}{2 \pi \sigma},
\end{eqnarray*}
...
\begin{eqnarray}
 s \, \tilde{\xi}(q) - 1 & = & - D_1 q^2 \tilde{\xi}(q) - k_+ \tilde{\xi}(z_0) - k_d \tilde{\xi}(q) + k_a \tilde{c}(q) \elabel{FDE_dna} \\
 s \, \tilde{c}(q,{\mathbf k}) & = & - D_3 (q^2 + k^2) \tilde{c}(q, \mathbf k \mathrm) - \left[ k_a \tilde{c}(q) - k_d \tilde{\xi}(q) \right] J_{0}(k \sigma). 
 \elabel{FDE_bulk}
\end{eqnarray}
...
\end{eqnarray}
Solving the above for $\tilde{c}(q)$, and substituting into \eref{FDE_dna}, we solve it for $\tilde{\xi}(q)$. Again, back-transforming this equation in $q$, we find
\begin{equation}
