...
We want to be able to model promoter site's in \GFRD. A promoter site is simulated as a sink situated on a rod. A particle diffusing along this rod, can, as it walks over the sink, either react with it or just move over it. Their is thus a reactive flux proportional to the probability of the particle being at the sink and the rate constant $k$. Next to the sink, the full \GFRD\, domain will also contain two absorbing boundaries. The fluxes at these three points are used to determine the event type at the next event time.
...
For completes and its usefulness in \GFRD, we end this derivation with the Green's function for an absorbing sink at $x_s$ in an infinite domain
\begin{multline}
