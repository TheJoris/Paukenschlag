...
\end{equation}
such that it can be Laplace transformed, and has correct behaviour in the long time limit. We will use the approximation in the limit of small $s$, and thus in large $t$. Because the argument of the exponential will diverge for large $t$, a small argument expansion will not work. We can, however, make a large time approximation by replacing $k_{{\rm irr}}(t)$ with $k_{{\rm irr}}(t \to \infty)$. Since a particle starting at the promoter is able to escape, $S(t|\sigma)$ approaches a constant value at large $t$. Therefore, the system reaches a steady-state limit $k_{{\rm irr}}>0$. The survival probability becomes  
\begin{equation}
